...
class ApplyController extends AuthController
{
    ...
    public function update()
    {
        try {
            ...
            if ($data['id']) {
                ...
                return $this->writeJson(0, null, '编辑应用信息成功');
            } else {
                //新增
                $data['created_at'] = Carbon::now();
                $model->data($data)->save();
                return $this->writeJson(0, null, '新增应用信息成功');
            }
        } catch (\Throwable $e) {
            return $this->writeJson(1, null, $e->getMessage());
        }
    ...
    public function lock()
    {
        try {
            ...
            $info->update(['status' => Applys::STATUS_2]);
    ...
    public function active()
    {
        try {
            ...
            $info = $model->get($id);
            if (empty($info)) return $this->writeJson(1,null,'抱歉，你要操作的信息不存在');
            $info->update(['status' => Applys::STATUS_1]);
            return $this->writeJson(0);
        } catch (\Throwable $e) {
            return $this->writeJson(1, null, $e->getMessage());
        }
